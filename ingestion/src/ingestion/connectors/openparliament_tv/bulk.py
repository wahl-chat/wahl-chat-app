...
Usage (local dev):
    cd ingestion
    uv run python -m ingestion.connectors.openparliament_tv.bulk --help
    uv run python -m ingestion.connectors.openparliament_tv.bulk --from-github --limit 20
    QDRANT_URL=http://localhost:6333 ENV=dev \\
        uv run python -m ingestion.connectors.openparliament_tv.bulk --from-github
...
if __name__ == "__main__":
    ...
    if not _bulk_env_path.exists():
        _bulk_env_path = _bulk_env_path.parents[1] / "ai-backend" / ".env"
...
from ingestion.connectors.openparliament_tv.client import OpTvClient
from ingestion.connectors.openparliament_tv.mappers.corpus import (
    build_chunk_records,
)
from ingestion.connectors.openparliament_tv.supersede import (
...
from ingestion.speech_dedup import datum_to_external_id as _datum_to_external_id
...
# MdB-Stammdaten XML, resolved relative to ingestion/ (bulk.py is 4 levels deep).
