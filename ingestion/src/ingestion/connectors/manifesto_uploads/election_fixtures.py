...
Not in the container image (only ``ingestion/`` is copied in), so a Cloud Run Job
...
from wahlchat_common.governance_levels import EU, FEDERAL, MUNICIPAL, STATE
...
def load_election(context_id: str, env: Optional[str] = None) -> ElectionFixture:
    ...
    if source == "firestore":
        ...
        from ingestion.connectors.manifesto_uploads.firestore_fixtures import (  # noqa: PLC0415
