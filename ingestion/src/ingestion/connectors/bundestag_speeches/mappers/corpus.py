...
Exports:
    ...
    chunk_text(text, ...)        — shared boundary-aware character splitter
                                   (re-exported from ingestion.chunking)
...
from ingestion.schemas import AuthorityTier, ChunkRecord, SpeechMeta, SourceType
from ingestion.speech_key import (
...
def compute_speech_key(speech: dict) -> Optional[str]:
    ...
    Uses the SAME ``ingestion/src/ingestion/speech_key.py`` helper op uses so the two sources
...
# major-party core (incl. the CDU/CSU rule — see ingestion/src/ingestion/party_slugs.py)
...
# ---------------------------------------------------------------------------
# Chunking is centralised: chunk_text is imported from ingestion.chunking
