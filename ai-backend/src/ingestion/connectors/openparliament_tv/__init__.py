...
plus a video deep-link payload (D-01/D-02/D-03).

Package structure (mirrors connectors/bundestag_speeches/):
  connector.py        — OpenParliamentTvConnector(BaseConnector): discover/fetch/normalize (11-04)
  client.py           — OpTvClient: keyless GitHub-bulk enumerate + pinned-host raw fetch, curl fallback
  parser.py           — parse_session_json: session JSON → speech dicts w/ D-03 gate + sentence_map
